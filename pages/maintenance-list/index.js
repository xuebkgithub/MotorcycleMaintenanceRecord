...
Page({
  ...
  onShow() {
    ...
    const pendingId = wx.getStorageSync('pendingMaintenanceDetailId');
    if (pendingId) {
      // 清除缓存
      wx.removeStorageSync('pendingMaintenanceDetailId');

      // 延迟一下再跳转，确保列表页已加载完成
      setTimeout(() => {
        wx.navigateTo({
          url: `/pages/maintenance-detail/index?id=${pendingId}`
        });
      }, 100);
    }
  },
  ...
  loadData() {
    ...
    // 读取所有保养记录
    let allRecords = storage.getMaintenanceRecords();
    ...
    const totalMileage = calculator.getTotalMileage(sortedRecords);
  ...
  onRecordTap(e) {
    ...
    wx.navigateTo({
      url: `/pages/maintenance-detail/index?id=${record.id}`
    });
  },
  ...
  onFabClick() {
    wx.navigateTo({
      url: '/pages/maintenance-add/index'
    });
  },
